...
function generateSKU(index) {
  return `BAG-${String(index + 1).padStart(4, '0')}`;
}
...
function createTables() {
  db.exec(`CREATE TABLE IF NOT EXISTS products (
    ...
    category TEXT DEFAULT 'Handbags',
    images TEXT,
    source_url TEXT,
    moq TEXT,
...
function insertProduct(product, index) {
  ...
  const comparePrice = price * 1.3; // 设置对比价格为原价的1.3倍
  ...
  stmt.run(
    sku,
    product.subject,
    `High quality ${product.subject.toLowerCase()}. Perfect for daily use and special occasions.`,
    price,
    comparePrice,
    100,
    'Handbags',
  ...
  console.log(`✓ Imported: ${product.subject.substring(0, 50)}...`);
}
...
function main() {
  try {
    ...
    for (let i = 0; i < alibabaProducts.length; i++) {
      insertProduct(alibabaProducts[i], i);
    }
    
    console.log(`\n✅ Successfully imported ${alibabaProducts.length} products!`);
    console.log('\nProducts are now available on your website.');
