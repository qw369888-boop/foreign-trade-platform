...
// 读取完整的164个产品数据
const all168Products = JSON.parse(fs.readFileSync(path.join(__dirname, '../all-168-products.json'), 'utf8'));
...
app.get('/api/products', (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: products,
      total: all168Products.length,
      page: Math.floor(offsetNum / limitNum) + 1,
      totalPages: Math.ceil(all168Products.length / limitNum)
    });
  } catch (error) {
...
app.get('/api/products/:id', (req, res) => {
  try {
    ...
    const product = all168Products.find(p => p.id === id);
...
app.get('/api/categories', (req, res) => {
  try {
    const categories = [...new Set(all168Products.map(p => p.category))];
    
    res.json({
      success: true,
      data: categories.map(cat => ({
        name: cat,
        count: all168Products.filter(p => p.category === cat).length
      }))
    });
  } catch (error) {
...
app.post('/api/cart/add', (req, res) => {
  try {
    ...
    const product = all168Products.find(p => p.id === parseInt(productId));
...
app.listen(PORT, () => {
  ...
  console.log(`📊 COMPLETE DATABASE loaded with ${all168Products.length} products`);
  ...
  console.log(`💰 Real market pricing: $8.62 - $54.33`);
  console.log(`📦 MOQ: 100 Pieces for all products`);
  console.log(`⚡ Ready to serve API requests`);
  
  // 显示分类统计
  const categoryStats = {};
  all168Products.forEach(product => {
    categoryStats[product.category] = (categoryStats[product.category] || 0) + 1;
  });
  
  console.log('\n📊 Product Categories:');
  Object.entries(categoryStats).forEach(([category, count]) => {
    const categoryProducts = all168Products.filter(p => p.category === category);
    ...
    console.log(`🏷️  ${category}: ${count} products ($${minPrice.toFixed(2)} - $${maxPrice.toFixed(2)}, 平均: $${avgPrice.toFixed(2)})`);
  });
  
  const allPrices = all168Products.map(p => p.price);
  console.log(`\n💰 Overall Price Range: $${Math.min(...allPrices).toFixed(2)} - $${Math.max(...allPrices).toFixed(2)}`);
  console.log(`📈 Average Price: $${(allPrices.reduce((a, b) => a + b, 0) / allPrices.length).toFixed(2)}`);
  console.log(`✅ Total: ${all168Products.length} products across ${Object.keys(categoryStats).length} categories`);
});
