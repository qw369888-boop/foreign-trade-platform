...
// 读取真正的原版产品数据
const originalProducts = JSON.parse(fs.readFileSync(path.join(__dirname, '../real-products-final.json'), 'utf8'));
...
app.get('/api/products', (req, res) => {
  try {
    ...
    res.json({
      ...
      totalPages: Math.ceil(originalProducts.length / limitNum)
    });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      data: categories.map(cat => ({
        name: cat,
        count: originalProducts.filter(p => p.category === cat).length
      }))
    });
  } catch (error) {
...
app.post('/api/cart/add', (req, res) => {
  try {
    ...
    const product = originalProducts.find(p => p.id === parseInt(productId));
...
app.listen(PORT, () => {
  ...
  console.log(`📊 ORIGINAL product database loaded with ${originalProducts.length} products`);
  ...
  // 显示原版产品列表
  console.log('\n📦 ORIGINAL Products from Database:');
  originalProducts.forEach((product, i) => {
    console.log(`${i + 1}. ${product.name}`);
    console.log(`   中文: ${product.name_zh}`);
    console.log(`   SKU: ${product.sku} | 价格: $${product.price} | MOQ: ${product.moq}`);
    console.log(`   分类: ${product.category} | 库存: ${product.stock}`);
    console.log('');
  });
});
